...
function normalizeTransport(raw: any): HotelTransportInfo[] {
  ...
  // Migrate from legacy flat object { metro, bus, drive }
...
function normalizeHotelConfig(hotel: any): HotelConfig {
  const empty = createEmptyHotel()

  // Migrate legacy room_types array → single room_type + price
  ...
  // Migrate legacy map { lat, lng } → map_image / map_url
  ...
  // Normalize weather values to numbers
...
function normalizeSignupFlow(extra: Partial<ActivityExtraConfig>, defaults: SignupFlowConfig): SignupFlowConfig {
  ...
  return {
    steps: ensureBuiltinSteps(fallbackSteps),
  }
}

function sanitizeSignupFlow(flow: SignupFlowConfig): SignupFlowConfig {
  ...
  return {
    steps: ensureBuiltinSteps(sanitized),
  }
}
  ...
  return {
    ...
    extra: {
      ...
      agenda_blocks: Array.isArray(extra.agenda_blocks) && extra.agenda_blocks.length
        ...
        : [createEmptyAgendaDay()],
      hotels: Array.isArray(extra.hotels) && extra.hotels.length
        ? extra.hotels.map((hotel: any) => normalizeHotelConfig(hotel))
        : [createEmptyHotel()],
