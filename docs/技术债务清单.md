...
**问题描述**:
评论链接线UI效果仍存在显示问题，当前先记录为技术债务。后续在页面重构阶段统一优化调整，保证评论层级关系与视觉连线的一致性。

**处理策略**:
1. 先完成「我的」页面重构（优先处理「活动列表」页）
2. 重构完成后回到评论模块统一优化链接线样式与布局
3. 对不同评论层级、长文本、折叠/展开状态进行回归验证

**优先级理由**:
- 当前存在视觉问题，但不阻塞核心流程使用
- 即将进行页面重构，适合在样式体系稳定后统一处理

**相关文件**:
- `frontend/app/src/pages/comment/components/CommentList.tsx`
- `frontend/app/src/pages/comment/components/ReplyPanel.tsx`
...
**问题描述**:
活动卡片展开区中「待缴费」「待签到」「完善交通信息」等标签/文案在小屏设备上文字被挤压，需要优化布局。

**理想方案**:
1. 三点菜单改为纵向排列（⋮）节省横向空间
2. 标签改为自适应换行或缩小字号
3. 参与人行使用 flex-wrap 或多行布局

**相关文件**:
- `frontend/app/src/pages/profile/components/ActivitiesTab.tsx`
- `frontend/app/src/pages/profile/components/ActivitiesTab.scss`

### #TD-007: 签到/签到成功页UI创意优化
...
**问题描述**:
当前签到页和签到成功页基本功能已实现（渐变圆圈 + 同心圆），但用户希望有更好的创意UI方案。

**优化方向**:
1. 签到页：更具动感的按钮动效、品牌氛围强化
2. 签到成功页：成功动画（粒子/纹波）、信息卡片排版优化

**相关文件**:
- `frontend/app/src/pages/checkin/`
- `frontend/app/src/pages/checkin-success/`
...
| 2026年2月25日 00:00 | 新增 | #TD-006 | 评论链接线UI优化调整 |
| 2026年2月26日 10:45 | 新增 | #TD-007 | 活动卡片展开区标签/文案空间不足 |
| 2026年2月26日 10:45 | 新增 | #TD-008 | 签到/签到成功页UI创意优化 |
