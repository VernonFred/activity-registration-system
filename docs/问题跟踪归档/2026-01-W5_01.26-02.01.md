...
1. **类型更新** (`types.ts`)
   - `Reply` 接口添加 `replies?: Reply[]` 支持递归嵌套
   - 添加 `like_count` 和 `is_liked` 字段

2. **ReplyPanel.tsx 重写**
   - 使用 Grid 布局 + Thread 结构
   - 主评论 `.thread.main` (64rpx头像)
   - 回复 `.thread.reply` (48rpx头像)
   - 递归渲染函数 `renderReplyItem` 支持无限嵌套
   - 每个回复项都有完整的点赞/拉踩/回复/编辑/删除功能
   - Mock数据展示3级嵌套结构

3. **ReplyPanel.scss 重写**
   - `.replies-level-1/2/3` 分层回复区域
   - 每层有独立的负边距和padding对齐主干线
   - 弯曲连接线使用 `::before` 伪元素
   - 主干线 `.line` 由父级头像列控制
   - `border-bottom-left-radius` 实现平滑弧线
