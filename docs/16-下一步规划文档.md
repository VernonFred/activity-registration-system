...
**已完成功能** (小程序端 100% 基本完成):
- ✅ 前端全部 18 个页面：首页、搜索、活动列表、活动详情(4Tab)、报名、评论、签到、凭证、议程、缴费、发票抬头、徽章墙、AI助手、个人中心、设置(语言/暗黑模式)
- ✅ i18n 双语支持：简体中文 + English，300+ 翻译 key，全页面覆盖
- ✅ 双主题适配：所有页面支持浅色/暗黑模式
- ✅ Mock 数据系统：所有接口均有 Mock 实现（含缴费/发票抬头）
- ✅ 后端 API：100% 完成（Flask + PostgreSQL）
- ✅ 微信登录鉴权：已实现

**当前阶段**:
- 🔧 **管理后台 UI 重构**：已有部分页面和功能，需全面重构 UI 样式
- ⚠️ **技术债务**: 测试覆盖率极低 < 5%（P2）
- ⚠️ **技术债务**: TypeScript `any` 滥用 150+ 处（P2）

### 当前专项（2026年3月）

**管理后台 UI 重构与管理能力补全**

- 目标：在不推翻现有功能架构的前提下，全面重构管理后台视觉系统，补齐空壳页与缺失页
- 执行原则：先定后台 API 契约，再前后端并行；设计稿仅参考功能逻辑与结构，不做像素级照搬
- 当前重点：冻结后台路由/菜单/页面边界，并补充管理员侧服务契约骨架

### 当前 P0（2026年3月3日）

**P0：管理后台-活动创建页面重构（与小程序端活动详情/报名流程对齐）**

- 将 `ActivityCreate` 从长表单重构为顶部分栏 Tabs
- 报名字段与报名流程规则统一收口到 `表单设计`
- 第一版继续复用现有 `/activities` 接口，不新增后端接口
- 当前执行到：`表单设计` 工作台收口
- 当前新增任务：
  - `FormDesigner` 重排为 `报名步骤 / 控件库 + 表单画布 / 实时预览`
  - 修复步骤切换、控件添加、字段编辑等核心交互
  - 补齐浅色/暗黑双主题适配
...
> 以下旧的“四周详细规划”主要对应小程序端历史阶段；当前执行以“管理后台专项”优先。
...
- 将 `Notifications` / `Scheduler` 纳入侧边栏
...
- 重构 `AdminLayout.tsx`
- 统一通用组件：`PageHeader / SectionCard / StatCard / FilterBar`

### 阶段 2：核心页面重构
- 优先：`Dashboard`
- 其次：`Activities / ActivityCreate / ActivityDetail`
- 然后：`SignupManage`
- 最后统一刷新：`BadgeRules / Notifications / Scheduler / FormDesigner`
...
### 阶段 1：Tabs 架构与状态模型
- 重构 `ActivityCreate` 为顶部分栏 Tabs
- 建立统一 `formState`
- 新增转换层：提交与回填都走同一套序列化逻辑
...
### 阶段 3：payload 转换 / 编辑回填
- 顶层字段继续写入活动基础字段
- 议程、酒店等结构化内容写入 `extra`
- 图片直播写入 `materials`
- 报名字段与报名流程继续由 `表单设计` 页统一维护

### 阶段 4：真实接口联调与回归
- 验证 `POST /activities`
- 验证 `PATCH /activities/{id}`
- 验证 `GET /activities/{id}` 回填
- 回归：不影响 `表单设计 / 报名管理 / 活动列表`

### 当前收口顺序（2026年3月6日）
- `表单设计 > 工作台布局`
- `表单设计 > 浅色/暗黑主题`
- `表单设计 > 控件交互恢复`
- `活动创建` 其余 Tab 继续按既定顺序推进
...
## 2026年3月6日当前重点（管理后台与代码治理）

### P0：FormDesigner 与报名流程链路收口

- 完成 `FormDesigner` 的交互和样式收口
- 按动态步骤模型验证：管理端配置 → 小程序报名 → `/registrations` 提交
- 继续修正深浅模式、拖拽、预览一致性问题

### P0：超长文件拆分治理

依据 `docs/2-代码规范文档.md`，单文件超过 300 行已视为明显超标。当前优先拆分目标：

1. `frontend/admin/src/styles/index.css`
2. `frontend/admin/src/pages/FormDesigner.tsx`
3. `frontend/app/src/pages/activity-detail/index.tsx`
4. `frontend/admin/src/pages/SignupManage.tsx`
5. `frontend/admin/src/pages/activity-create/transform.ts`
6. `frontend/admin/src/pages/activity-create/components/HotelTab.tsx`

### 拆分原则

- 页面容器、视图组件、表单配置、预览渲染、数据转换必须分层
- 样式文件拆成页面级、组件级、主题覆盖级，避免继续集中到 `index.css`
- 不接受继续在 500 行以上文件上叠加新功能

### 本阶段不做

- 不继续新增大页面功能，先完成结构治理
- 不再把新样式直接堆入 `frontend/admin/src/styles/index.css`


## 2026年3月6日规划修订（代码规范治理完成）
...
1. 动态报名步骤链路端到端回归
2. 管理后台 `FormDesigner` 与 `ActivityCreate` 细节收口
3. 报名、评论、支付、发票四条用户链路回归验证
